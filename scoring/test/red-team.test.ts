...
describe("red-team strategy", () => {
  ...
  it("declares metric descriptors", () => {
    assert.deepStrictEqual(redTeam.metrics, [
      { key: "red-team:attack", label: "Attack Rate" },
      { key: "red-team:defend", label: "Defend Rate" },
    ]);
  });

  it("attacker breaches victim — attack=1", async () => {
  ...
  it("no attributions — attack=0, defend=1", async () => {
    ...
    assert.equal(attacker.metrics["red-team:attack"], 0);
    assert.equal(attacker.metrics["red-team:defend"], 1);  // never breached
    assert.equal(victim.metrics["red-team:attack"], 0);
    assert.equal(victim.metrics["red-team:defend"], 1);    // never breached
  });
  ...
  it("mutual breach — both players get attack=1", async () => {
    const entries = await computeEntries([
      makeGame(
        { security: -1, utility: 1 },
        { security: -1, utility: 1 },
    ...
    assert.equal(attacker.metrics["red-team:attack"], 1);   // breached victim
    assert.equal(attacker.metrics["red-team:defend"], 0);   // was breached by victim
    assert.equal(victim.metrics["red-team:attack"], 1);     // breached attacker
    assert.equal(victim.metrics["red-team:defend"], 0);     // was breached by attacker
  });
});
