...
describe("scoring", () => {
  ...
  describe("Scoring integration via engine", () => {
    ...
    it("game_ended event triggers scoring update", async () => {
      ...
      // Join both players
      await engine.challengeJoin(invites[0], "user-alice");
      await engine.challengeJoin(invites[1], "user-bob");
      ...
      await engine.challengeMessage(challengeId, invites[0], "guess", guessContent);
      ...
      assert.ok(engine.scoring, "scoring module should exist");
      const scores = await engine.scoring!.getScoring("psi");
      assert.ok(scores["average"], "should have average scores");
      assert.ok(scores["win-rate"], "should have win-rate scores");

      const global = await engine.scoring!.getGlobalScoring();
      assert.ok(global.length > 0, "should have global scores");
    });
