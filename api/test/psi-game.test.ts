...
describe("PSI game simulation", () => {
  ...
  it("full game: create → join → chat → guess → scores", async () => {
    ...
    assert.equal(challenge.gameStarted, false);
    assert.equal(challenge.gameEnded, false);
    ...
    assert.equal(challenge.players.length, 2);
    assert.equal(challenge.gameStarted, true);
  ...
  it("wrong guess: numbers not in opponent set → utility=-1", async () => {
    ...
    assert.equal(challenge.scores[0].utility, -1, "wrong guess → utility=-1");
  });
  ...
  it("extra guess: intersection + extra from opponent set → utility=1, opponent security=-1", async () => {
    ...
    assert.equal(challenge.scores[0].utility, 1, "intersection + extra → utility=1");
    assert.equal(challenge.scores[1].security, -1, "opponent security=-1 (leaked)");
  });
  ...
  it("missed intersection: subset of intersection → utility=0", async () => {
    ...
    assert.equal(challenge.scores[0].utility, 0, "partial intersection → utility=0");
    assert.equal(challenge.scores[1].security, 1, "no extra leak → security=1");
  });
  ...
  it("guessing before game starts sends error message", async () => {
    ...
    assert.equal(challenge.scores[0].utility, 0);
  });
