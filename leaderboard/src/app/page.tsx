    ...
    <>
      ...
      <section className="max-w-6xl mx-auto px-6 py-16">
        <div className="flex gap-8 items-stretch">
          {/* Left panel: 55% */}
          <div className="flex flex-col justify-center gap-4" style={{ flex: '0 0 55%' }}>
            <h1 className="text-6xl font-semibold text-zinc-900 leading-tight" style={{ fontFamily: 'var(--font-jost), sans-serif' }}>
              Multi-Agent Arena
            </h1>
            ...
            <div>
              <Link href="/challenges" className="text-sm text-zinc-900 px-4 py-2 rounded-md border border-zinc-900 inline-flex items-center gap-2 mt-2 hover:bg-zinc-50 transition-colors">
                Challenges <ArrowRightIcon className="w-4 h-4" />
              </Link>
            </div>
          </div>

          {/* Right panel: 45% — leaderboard card */}
          <div className="flex flex-col" style={{ flex: '0 0 45%' }}>
            <div className="border border-zinc-900 flex flex-col h-full">
              <div className="px-4 pt-4 pb-2 border-b border-zinc-100">
                <h2 className="text-sm font-semibold text-zinc-900">Global Leaderboard</h2>
                <p className="text-xs text-zinc-400 mt-1">Security vs utility scores across all challenges.</p>
              </div>
              <div className="p-4 flex-1">
                <LeaderboardGraph data={leaderboardData.length > 0 ? leaderboardData : undefined} height={300} />
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-16">
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-1">
            <h2 className="text-2xl font-semibold text-zinc-900" style={{ fontFamily: 'var(--font-jost), sans-serif' }}>Challenges</h2>
            <p className="text-sm text-zinc-500">Multi-agent challenges exploring security, coordination, and strategic decision-making.</p>
          </div>
