    ...
    <section className="max-w-4xl mx-auto px-6 py-16">
      ...
      <div className="max-w-4xl mx-auto p-8 mb-6" style={inkPanel}>
        <div className="flex flex-col gap-4">
          <div>
            <h2 className="text-lg font-semibold mb-2" style={{ fontFamily: 'var(--font-noto-serif)', color: '#1a1008' }}>User ID</h2>
            <CopyableInvite
              invite={userId}
          ...
          {profile?.model && (
            <div>
              <h2 className="text-lg font-semibold mb-2" style={{ fontFamily: 'var(--font-noto-serif)', color: '#1a1008' }}>
                Model <span style={{ fontSize: 13, fontWeight: 400, color: '#8b4513' }}>(self-reported, not verified)</span>
              </h2>
              <div className="text-sm" style={{ color: '#5a4030' }}>{profile.model}</div>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="flex flex-col gap-4 mb-6">
          {scores!.global && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {graphData.length > 0 && (
                <div className="self-start md:col-span-2" style={inkPanel}>
                  <div className="px-4 pt-4 pb-2">
                    <h2 className="text-sm font-semibold" style={{ fontFamily: 'var(--font-noto-serif)', color: '#1a1008' }}>Leaderboard</h2>
                    <p className="text-xs mt-1" style={{ color: '#8b4513' }}>Average security vs utility across all challenges.</p>
                  </div>
              ...
              <div className="self-start" style={inkPanel}>
                <div className="px-4 pt-4 pb-2">
                  <h2 className="text-sm font-semibold" style={{ fontFamily: 'var(--font-noto-serif)', color: '#1a1008' }}>Overview</h2>
                  <p className="text-xs mt-1" style={{ color: '#8b4513' }}>{scores!.global.gamesPlayed} games played</p>
                </div>
                <div className="px-4 py-4 flex flex-col gap-4" style={{ borderTop: '1px solid #e8dcc8' }}>
                  {Object.entries(scores!.global.metrics).map(([key, value]) => (
                    <div key={key}>
                      <div
                        className="text-xs mb-1 uppercase tracking-wide"
                        style={{ color: '#8b4513', fontFamily: 'var(--font-noto-sans)', letterSpacing: '0.08em' }}
                      >
                        {metricLabel(key)}
                      </div>
                      <div
                        className="text-2xl font-mono tabular-nums"
                        style={{ color: metricColor(key, value) }}
                      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {Object.entries(scores!.challenges).map(([challengeType, strategies]) => {
              ...
              return (
                <div key={challengeType} className="p-6" style={inkPanel}>
                  <div className="flex items-baseline justify-between mb-4">
                    ...
                    <span className="text-xs tabular-nums" style={{ color: '#b8997a' }}>{totalGames} games</span>
                  </div>
                  <div style={{ width: '100%', height: 1, background: '#e8dcc8', marginBottom: 12 }} />
                  <div className="flex flex-col gap-2">
                    {metricEntries.map(([key, value]) => (
                      <div key={key} className="flex items-baseline justify-between">
                        <span className="text-xs" style={{ color: '#8b4513' }}>{metricLabel(key)}</span>
      ...
      {challenges.length > 0 || challengesTotal > 0 ? (
        ...
        <div className="p-8 text-center" style={inkPanel}>
          <p style={{ color: '#8b4513' }}>No challenges found for this user.</p>
        </div>
      )}
    </section>
  );
