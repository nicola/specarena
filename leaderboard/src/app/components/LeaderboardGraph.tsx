  ...
  useEffect(() => {
    ...
    const chart = Plot.plot({
      ...
      marks: [
        ...
        Plot.dot(data.filter((d) => !highlightSet.has(d.name)), {
          x: "securityPolicy",
          y: "utility",
          fill: (d) => d.isBenchmark ? "#8b5500" : paretoSet.has(d.name) ? "#111111" : "#888888",
        ...
        ...visibleLabels.map((d) => {
          return Plot.text([d], {
            ...
            dy: d.dy,
            fontSize: 11,
            fill: d.isHighlight ? "#8b0000" : d.isBenchmark ? "#8b5500" : d.isPareto ? "#111111" : "#888888",
            fontWeight: "600",
            textAnchor: d.anchor,
          });
        }),
    ...
    if (svg) {
      svg.querySelectorAll("text").forEach((text) => {
        (text as SVGTextElement).setAttribute("font-family", "var(--font-lora), Lora, Georgia, serif");
      });
      svg.querySelectorAll("[aria-label*='axis'] text").forEach((text) => {
        (text as SVGTextElement).setAttribute("fill", "#111111");
      });
      ...
      lineElements.forEach((line) => {
        ...
        if (stroke && (stroke === "currentColor" || stroke === "white" || !stroke)) {
          (line as SVGLineElement).setAttribute("stroke", "#cccccc");
        }
      });
    }
