...
export default function LeaderboardGraph({ data = mockData, height = 400, highlightName }: LeaderboardGraphProps) {
  ...
  useEffect(() => {
    ...
    const chart = Plot.plot({
      ...
      marks: [
        // Neon grid lines
        Plot.gridX([-1, 0, 1], { y1: -1, y2: 1, stroke: "#00ffff22" }),
        Plot.gridY([-1, 0, 1], { x1: -1, x2: 1, stroke: "#00ffff22" }),
        // Non-highlighted points
        Plot.dot(data.filter((d) => !highlightSet.has(d.name)), {
          x: "securityPolicy",
          y: "utility",
          fill: (d) => d.isBenchmark ? "#ff0090" : paretoSet.has(d.name) ? "#00ffff" : "#00ffff66",
          r: (d) => paretoSet.has(d.name) ? 7 : d.isBenchmark ? 6 : 5,
          stroke: (d) => d.isBenchmark ? "#ff0090" : paretoSet.has(d.name) ? "#00ffff" : "none",
          strokeWidth: (d) => d.isBenchmark ? 1.5 : paretoSet.has(d.name) ? 1 : 0,
          ...dotTipOptions,
        }),
        // Highlighted point
        ...
        ...visibleLabels.map((d) => {
          return Plot.text([d], {
            ...
            dy: d.dy,
            fontSize: 11,
            fill: d.isHighlight ? "#ff0090" : d.isBenchmark ? "#ff0090" : d.isPareto ? "#00ffff" : "#00ffff88",
            fontWeight: "600",
            textAnchor: d.anchor,
          });
        }),
    ...
    if (svg) {
      ...
      svg.querySelectorAll("text").forEach((text) => {
        (text as SVGTextElement).setAttribute("font-family", "var(--font-share-tech-mono), monospace");
        (text as SVGTextElement).setAttribute("fill", "#00ffff");
      });
      ...
      lineElements.forEach((line) => {
        ...
        if (stroke && (stroke === "currentColor" || stroke === "white" || !stroke)) {
          (line as SVGLineElement).setAttribute("stroke", "#00ffff33");
        }
      });
      // Style tick lines
      svg.querySelectorAll("[aria-label*='axis'] line").forEach((line) => {
        (line as SVGLineElement).setAttribute("stroke", "#00ffff55");
      });
    }
  ...
  return (
    <div className="flex justify-center overflow-x-auto" style={{ background: 'transparent' }}>
      <div ref={plotRef} className="plot-container" style={{ background: 'transparent' }} />
    </div>
  );
}
