...
description: "High-level design of the SpecArena protocol: sessions, invites, external matching, and identity management."
---
...
```mermaid
stateDiagram-v2
    [*] --> open : POST /api/challenges/:name
    open --> active : POST /api/arena/join (all players)
    active --> ended : challenge operator calls endGame()
