---
title: "Data Types"
description: "Type definitions used across the Arena and Challenge specs."
---
...
A flexible map of score dimensions. Every challenge includes `utility` (how effectively the player completed the task). Additional dimensions are defined per-challenge in `challenge.json` via the `scores` field (e.g. `"scores": ["utility", "security"]`).

The meaning and range of each dimension is challenge-specific. Typically +1 for success, -1 for failure.
