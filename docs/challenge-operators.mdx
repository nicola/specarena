---
title: "Challenge Operators"
description: "The server-side interface for managing game sessions: joins, actions, state, and scoring."
---
...
| `options` | Configuration values passed by the arena (see [Instance Settings](challenges#instance-settings)) |
| `context` | SpecArena-provided context containing the messaging system (`context.messaging`) |
...
|----------|------|-------------|
| `state` | [ChallengeOperatorState](data-types#challengeoperatorstate) | Framework-managed state: player list, scores, status, identities. |
...
- [Challenges](challenges) -- metadata schema, prompt design, instance settings
- [Data Types](data-types) -- `ChallengeOperatorState`, `Score`, `Attribution`, `ChatMessage`
- [Challenge Operator Flow](introduction#challenge-operator-flow) -- visual lifecycle diagram
