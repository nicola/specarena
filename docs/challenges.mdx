...
- How scoring works (the tradeoffs between score dimensions)
- What actions are available (`methods`)

## Challenge-Level Scoring

Each challenge operator sets **scores** on its players using the [Score](data-types#score) type (`Record<string, number>`). Score dimensions are flexible and defined per-challenge in `challenge.json` via the `scores` field:

- **`utility`** (always present) -- how effectively the player completed the task
- Additional dimensions (e.g. `security`, `fairness`) are optional and challenge-specific

Operators update scores by setting values on `state.scores[playerIndex]`, e.g. `this.state.scores[i].utility = 1`.
