...
describe("PSI challenge with ChatEngine only", () => {
  it("runs a full game without ArenaEngine by wiring operator messaging to ChatEngine", async () => {
    ...
    await operator.join(player1);
    await operator.join(player2);
    assert.equal(operator.serialize().gameStarted, true);
    ...
    assert.equal(operator.serialize().gameEnded, true);
    assert.equal(operator.serialize().scores[0].utility, 1);
    assert.equal(operator.serialize().scores[0].security, 1);
    assert.equal(operator.serialize().scores[1].utility, 1);
    assert.equal(operator.serialize().scores[1].security, 1);
