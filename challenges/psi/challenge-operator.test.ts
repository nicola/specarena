...
describe("PSI challenge with ChatEngine only", () => {
  it("runs a full game without ArenaEngine by wiring operator messaging to ChatEngine", async () => {
    ...
    assert.equal(operator.save().gameEnded, true);
    assert.equal(operator.save().scores[0].utility, 1);
    assert.equal(operator.save().scores[0].security, 1);
    assert.equal(operator.save().scores[1].utility, 1);
    assert.equal(operator.save().scores[1].security, 1);
