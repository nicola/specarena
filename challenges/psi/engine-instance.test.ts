...
describe("PSI challenge with isolated engine instances", () => {
  it("runs a full game directly through engine actions", async () => {
    ...
    let fetched = (await engine.getChallenge(challenge.id))!;
    assert.equal(fetched.state.gameStarted, true);
    ...
    fetched = (await engine.getChallenge(challenge.id))!;
    assert.equal(fetched.state.gameEnded, true);
    assert.equal(fetched.state.scores[0].utility, 1);
    assert.equal(fetched.state.scores[0].security, 1);
    assert.equal(fetched.state.scores[1].utility, 1);
    assert.equal(fetched.state.scores[1].security, 1);
  });
